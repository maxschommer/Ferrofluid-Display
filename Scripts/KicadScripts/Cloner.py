#!/usr/bin/env python
# Run: execfile("/home/maxschommer/Documents/Github/Ferrofluid-Display/Scripts/KicadScripts/Cloner.py")
...
templateReferences = ['U6001', 'L6001', 'D6001', 'R6001', 'R6002', 'D6002']
...
inputBoard = '/home/maxschommer/Documents/Github/Ferrofluid-Display/Electrical/ferrofluid_display/ferrofluid_display.kicad_pcb'
# Output file, original file remains unmodified
outputBoardFile = '/home/maxschommer/Documents/Github/Ferrofluid-Display/Electrical/ferrofluid_display/ferrofluid_display_scripted.kicad_pcb'

templateRefModulo = 1000  # Difference in the reference numbers between hierarchical sheet
templateRefStart = 200  # Starting point of numbering in the first hierarchical sheet
move_dx = FromMM(16)  # Spacing between clones in x direction
move_dy = FromMM(14)  # Spacing between clones in y direction
clonesX = 10  # Number of clones in x direction
clonesY = 10  # Number of clones in y direction
...
for templateRef in templateReferences:  # For each module in the template schema
    ...
    if templateModule is not None:
        ...
        i = 0
        missedRefs = 0
        while i < numberOfClones + missedRefs:  # Create list of references to be cloned of this module in the template
            if missedRefs > 100:
                break
            cloneRefNumber = int(templateReferenceNumber) + (i + 1) * templateRefModulo  # Number of the next clone
            cloneStringRef = re.sub(templateReferenceNumber, "", templateRef) + str(
                cloneRefNumber)

            checkCloneModule = board.FindModuleByReference(cloneStringRef)
            if checkCloneModule is None:
                missedRefs += 1
                i += 1
                continue
            cloneReferences.append(cloneStringRef)  # String reference of the next clone
            i += 1
...
for i in range(0, board.GetAreaCount()):  # For all the zones in the template board
    ...
    if templateRect.Contains(
            zone.GetPosition()) and zone.GetLayer() is not 41:  # If the zone is inside the area to be cloned (the comment zone) and it is not the comment zone (layer 41)
        for i in range(1, numberOfClones):  # For each target clone areas
            zoneClone = zone.Duplicate()  # Make copy of the zone to be cloned
            zoneClone.Move(
                wxPoint(i % clonesX * move_dx, i // clonesX * move_dy))  # Move it inside the target clone area
            for module in modules:  # Iterate through all the pads (also the cloned ones) in the board...
                for pad in module.Pads():
                    if zoneClone.HitTestInsideZone(pad.GetPosition()) and pad.IsOnLayer(
                            zoneClone.GetLayer()):  # To find the (last) one inside the cloned zone. pad.GetZoneConnection() could also be used
